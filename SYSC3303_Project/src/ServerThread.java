...
byte errornum = 1;
DatagramPacket err = FormError.IOerror("FILE_not_FOUND to READ",errornum);
       err.setAddress(request.getAddress());
       err.setPort(request.getPort());
    try {
    socket.send(err);
...
/*******************error 3*****************************/
long freespace = here.getFreeSpace();
if(freespace < temp.length){
byte errornum = 3;
...
byte errornum = 1;
DatagramPacket err = FormError.IOerror("FILE_not_FOUND to WRITE",errornum);
       err.setAddress(request.getAddress());
       err.setPort(request.getPort());
    try {
    socket.send(err);
...
byte errornum = 3;
DatagramPacket err = FormError.IOerror("NO ACCESS TO WRITE",errornum);
       err.setAddress(request.getAddress());
       err.setPort(request.getPort());
    try {
    socket.send(err);
    ...
    socket.close();
    System.out.println("File has no access to write");
    return;
